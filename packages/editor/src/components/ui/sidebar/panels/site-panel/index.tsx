          ...
          return (
            <div className="group/ref relative border-border/50 border-b" key="upload">
              ...
              >
                ...
                {uploading ? `上传 ${uploadingType}... ${progress}%` : '上传扫描/平面'}
              </button>
...
function ContentSection() {
  ...
  if (!level) {
    return (
      <div className="px-3 py-4 text-muted-foreground text-sm">请选择一个楼层以查看内容</div>
    )
  }
  ...
  if (elementChildren.length === 0) {
    return <div className="px-3 py-4 text-muted-foreground text-sm">该楼层暂无元素</div>
  }
    ...
    >
      ...
      >
        <div className="flex h-full min-w-0 flex-1 cursor-pointer items-center gap-2 py-2 pl-3">
          ...
          />
          <span className="truncate font-medium text-sm">{building.name || '建造'}</span>
        </div>
        <Popover
          ...
          >
            <div className="flex items-center gap-2">
              <img
                ...
                src="/icons/site.png"
              />
              <span className="font-medium text-sm">{siteNode.name || '站点'}</span>
            </div>
            <CameraPopover
              buttonClassName={cn(
                'transition-colors',
